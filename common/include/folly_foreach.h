...
#ifndef FOLLY_FOREACH_H_
#define FOLLY_FOREACH_H_
 ...
 */
#define FOR_EACH(i, c)                                                               \
    if (bool FOR_EACH_state1 = false) {                                              \
    } else                                                                           \
        for (auto&& FOR_EACH_state2 = (c); !FOR_EACH_state1; FOR_EACH_state1 = true) \
            for (auto i = FOR_EACH_state2.begin(); i != FOR_EACH_state2.end(); ++i)
...
#define FOR_EACH_R(i, c)                                                                   \
    if (bool FOR_EACH_R_state1 = false) {                                                  \
    } else                                                                                 \
        for (auto&& FOR_EACH_R_state2 = (c); !FOR_EACH_R_state1; FOR_EACH_R_state1 = true) \
            for (auto i = FOR_EACH_R_state2.rbegin(); i != FOR_EACH_R_state2.rend(); ++i)
...
#define FOR_EACH_ENUMERATE(count, i, c)                                              \
    if (bool FOR_EACH_state1 = false) {                                              \
        ...
        for (auto&& FOR_EACH_state2 = (c); !FOR_EACH_state1; FOR_EACH_state1 = true) \
            if (size_t FOR_EACH_privateCount = 0) {                                  \
            } else if (const size_t& count = FOR_EACH_privateCount) {                \
            } else                                                                   \
                for (auto i = FOR_EACH_state2.begin(); i != FOR_EACH_state2.end(); ++FOR_EACH_privateCount, ++i)
...
#define FOR_EACH_KV(k, v, c)                                                                               \
    if (unsigned int FOR_EACH_state1 = 0) {                                                                \
    } else                                                                                                 \
        for (auto&& FOR_EACH_state2 = (c); !FOR_EACH_state1; FOR_EACH_state1 = 1)                          \
            for (auto FOR_EACH_state3 = FOR_EACH_state2.begin(); FOR_EACH_state3 != FOR_EACH_state2.end(); \
                 FOR_EACH_state1 == 2 ? ((FOR_EACH_state1 = 0), ++FOR_EACH_state3)                         \
                                      : (FOR_EACH_state3 = FOR_EACH_state2.end()))                         \
                for (auto& k = FOR_EACH_state3->first; !FOR_EACH_state1; ++FOR_EACH_state1)                \
                    for (auto& v = FOR_EACH_state3->second; !FOR_EACH_state1; ++FOR_EACH_state1)
 ...
 */
template <class T, class U>
typename std::enable_if<HasLess<U, T>::value, bool>::type downTo(T& iter, const U& begin) {
    return begin < iter--;
}
